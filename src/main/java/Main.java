...
import exception.ExceptionCommand;
...
public class Main {
    ...
    public static void compiler() throws CloneNotSupportedException, IOException {
        ...
        try {
            if (!checkFile()) {
                System.out.println("Расширение файла должно быть .lua");
                System.exit(0);
            }
    ...
    public static void compile(Reader fileReader, String inputFile) throws IOException {
        ...
        nameAsm.deleteCharAt(inputFile.length() - 1);
        nameAsm.deleteCharAt(inputFile.length() - 2);

        nameAsm.setCharAt(inputFile.length() - 3, 's');
        ...
        nameProgram.deleteCharAt(inputFile.length() - 1);
        nameProgram.deleteCharAt(inputFile.length() - 2);
        nameProgram.deleteCharAt(inputFile.length() - 3);
        nameProgram.deleteCharAt(inputFile.length() - 4);
    ...
    public static void dumpAst(Reader fileReader, String outAst, String outSemaAst) throws CloneNotSupportedException, IOException {
        ...
        Semantic semantic = new Semantic(programTree, idTable.getIdTable());
        programTree.writeGraph(outSemaAst);
    ...
    public static void processingArguments(String[] args) throws ExceptionCommand {
        if (args.length == 2) {
            ...
            if (args.length == 1) {
                ...
                inputFile = "./examples/min_max_array.lua";
            }
        }
    }
    ...
    public static boolean checkFile() {
        return (inputFile.charAt(inputFile.length() - 1) == 'a' &&
                inputFile.charAt(inputFile.length() - 2) == 'u' &&
                inputFile.charAt(inputFile.length() - 3) == 'l' &&
                inputFile.charAt(inputFile.length() - 4) == '.');
    }
}
